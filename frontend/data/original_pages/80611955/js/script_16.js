(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
    ...
    $(document).ready(function () {
        ...
        //省宣埋点 Read
        yszSdkUtils.StUtil.ZMYDataCollection({
            event: yszSdkUtils.StUtil.EventType.Read,
    ...
    function wx_share(url2) {
        ...
        let url3 = (window.location.href).split("?")[0];
        ...
        const succallback = function (data) {
            try {
                wx.config({
                    ...
                    openTagList: ['wx-open-launch-app','wx-open-launch-weapp','wx-open-audio'],
                });
                wx.ready(function () {
                    wx.checkJsApi({
    ...
    function replaceElement(element, newElement) {
        if (!element || !element.parentNode) {
            console.error("replaceElement: The element or its parentNode is null.", element, newElement);
            return;
        }
        element.parentNode.replaceChild(newElement, element);
    }
    function createLaunchAppButton() {
        // 创建script标签
        const script = document.createElement('script');
        script.type = 'text/wxtag-template';
        script.id = 'launchAppScript';
        // 设置wx-open-launch-app的内容
        script.innerHTML = `
            <style>
                .launch-app-btn {
                    width:100vw;
            ...
            <div class="launch-app-btn">打开</div>
        `;
        
        // 创建wx-open-launch-app元素
        const launchApp = document.createElement('wx-open-launch-app');
        
        // 设置必要属性
        launchApp.setAttribute('username', 'wxff1f0c8244042666');
        launchApp.setAttribute('appid', 'wxff1f0c8244042666'); // iOS的App Store ID或Android的包名
        launchApp.setAttribute('style', 'position: absolute; top: 14px; right: 16px; z-index: 9; width: 64px; height: 28px; border-radius: 5px; background-color: #F51869; font-size: 14px; text-align: center; line-height: 28px; color: #FFFFFF; font-family: PingFangSC-regular; text-decoration: none;');
        
        // 可选：传递的额外参数
        launchApp.setAttribute('extinfo', "ysz://com.cutv.shakeshake/native/openApp");
        
        // 添加子元素
        launchApp.appendChild(script);
        replaceElement(document.querySelector('header.header aside .open'), launchApp);
        // 添加事件监听
        setTimeout(() => {
            attachLaunchAppEvents();
        }, 100);
        
    }
    
    // 绑定事件
    function attachLaunchAppEvents() {
        const launchApp = document.querySelector('wx-open-launch-app');
        if (launchApp) {
            // 监听成功事件
            launchApp.addEventListener('launch', function(e) {
                console.log('成功调起APP', e.detail);
                // 可以在这里处理成功调起后的逻辑
            });
            
            // 监听错误事件
            launchApp.addEventListener('error', function(e) {
                console.error('调起APP失败', e.detail);
                
                // 根据错误类型处理
                switch(e.detail.errMsg) {
                    case 'launch:fail':
                        // APP未安装或其他错误
                        alert('无法打开APP，请确认已安装应用');
                        break;
                    case 'launch:fail_check fail':
                        // 检查失败
                        alert('检查失败，请稍后重试');
                        break;
                    default:
    ...
    $("#addCommentBtn").click(async function () {
        ...
        //判断是否登录
        const userInfo = await yszSdkUtils.LoginUtil.check(true,null,null);
        if (!userInfo) {
            mui.toast('请先登录', { duration: 'long', type: 'div' });
            return;
        }
        const nickName = userInfo.UserInfo.NickName || '游客';
        const userAvatar = userInfo.UserInfo.Avatar || 'http://static.scms.sztv.com.cn/ysz/public_new/imgs/noavatar_middle.gif';
        ...
        const succallback = function (data) {
            ...
            if (data.returnCode === "0000") {
                ...
                html += '<div class="hot_head">' +
                    '<span class="hot_left">' +
                    '<img src="' + userAvatar + '" class="hot_img">' +
                    '</span>' +
                    '<span class="hot_right">' +
                    '<p class="hot_top">' +
                    '<span class="hot_title">' + nickName + '</span>' +
                    '</p>' +
                ...
                // 省宣埋点 评论统计
                yszSdkUtils.StUtil.ZMYDataCollection({
                    event: yszSdkUtils.StUtil.EventType.Comment,
                    article_id: $('#article').val(),
                    user_id: userid || ''
                });
            }
    ...
    function awakeApp(id) {
        const url = `ysz://com.cutv.shakeshake/native/articleDetail/detail?articleId=${id}`
        window.open(`https://app.sztv.com.cn/ysz/business?protocol=${btoa(url)}`, '_blank');
        return;
        /*
        if (id) {
            schemeUrl = "ysz://?articleId=" + id;
